...
function MedicalTransportLayer({ 
    ...
    const [ MedicalTransportVessels, setMedicalTransportVessels ] = useState<VesselPosition[]>([]);
    const debounceTimeoutRef = useRef<NodeJS.Timeout | null>(null);
    const currentViewportBoundsRef = useRef<ViewportBounds | undefined>(viewportBounds);

    const fetchMedicalTransportVessels = useCallback((bounds: ViewportBounds) => {
        const params = new URLSearchParams({
            minLat: bounds.minLat.toString(),
            maxLat: bounds.maxLat.toString(),
            minLon: bounds.minLon.toString(),
            maxLon: bounds.maxLon.toString(),
            vesselTypes: "58" // Medical Transport Vessels
        });
        ...
        fetch(`${api_url}/v1/vessels/position/all?${params}`)
            ...
            .then(data => setMedicalTransportVessels(data))
            .catch(error => console.error('Error fetching Medical Transport Vessels:', error));
    }, []);
    ...
    useEffect(() => {
        ...
        debounceTimeoutRef.current = setTimeout(() => {
            if (currentViewportBoundsRef.current) {
                fetchMedicalTransportVessels(currentViewportBoundsRef.current);
            }
    ...
    }, [visible, viewportBounds, fetchMedicalTransportVessels]);
    ...
    }, [visible, fetchMedicalTransportVessels]);
        

    return new IconLayer({
        id: "medical-transport-layer",
        data: MedicalTransportVessels,
        visible,
        pickable: true,
